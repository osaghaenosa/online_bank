...
export function PublicNav() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50" style={{ background: 'rgba(8,14,28,.85)', backdropFilter: 'blur(16px)', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <Link href="/dashboard" className="px-4 py-2 rounded-xl text-sm font-semibold text-white"
            ...
            <>
              ...
              <Link href="/auth/register" className="px-4 py-2 rounded-xl text-sm font-semibold text-white"
                style={{ background: '#10B981' }}>
                Open Account
              </Link>
            </>
      ...
      {/* Mobile menu */}
      {open && (
        <div className="md:hidden border-t px-6 py-4 space-y-3" style={{ background: 'rgba(8,14,28,.97)', borderColor: 'rgba(255,255,255,.08)' }}>
          {LINKS.map(l => (
          ...
          <div className="pt-3 border-t flex flex-col gap-2" style={{ borderColor: 'rgba(255,255,255,.1)' }}>
            <Link href="/auth/login" onClick={() => setOpen(false)}
              ...
              style={{ borderColor: 'rgba(255,255,255,.2)' }}>Sign In</Link>
            <Link href="/auth/register" onClick={() => setOpen(false)}
              className="text-center py-2.5 rounded-xl text-sm font-semibold text-white"
              style={{ background: '#10B981' }}>Open Account</Link>
          </div>
        </div>
      )}
    </nav>
