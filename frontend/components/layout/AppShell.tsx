...
import {
  ...
  ShieldCheck, Wallet, FileText, CreditCard,
} from 'lucide-react'
...
const PAGE_TITLES: Record<string, string> = {
  '/dashboard': 'Dashboard', '/account': 'Checking Account',
  '/deposit': 'Deposit Funds', '/withdraw': 'Withdraw Funds',
  '/transfer': 'Send Money', '/history': 'Transaction History',
  '/profile': 'My Profile', '/receipts': 'Receipt',
  '/admin': 'Admin Panel', '/admin/users': 'User Management',
  '/admin/transactions': 'Transactions', '/admin/settings': 'App Settings',
    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex-1 flex flex-col min-h-screen min-w-0">
        ...
        <header className="sticky top-0 z-30 flex items-center justify-between px-6 h-16 border-b"
          ...
          <div className="flex items-center gap-3">
            <button className="lg:hidden p-2 rounded-lg" style={{ background: 'var(--color-bg)' }}
              onClick={() => setOpen(true)}><Menu size={18} /></button>
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl text-sm font-semibold"
              ...
              <span className="font-mono font-bold" style={{ color: '#10B981' }}>{fmtUSD(user.balance)}</span>
            </div>
            {/* Bell */}
